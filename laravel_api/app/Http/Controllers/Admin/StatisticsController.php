...
class StatisticsController extends Controller
{
    ...
    public function getOrderStatusDistribution(Request $request)
    {
        ...
        $data = Order::whereBetween('created_at', [$startDate, $endDate])
            ->select('order_status as status', DB::raw('count(*) as count'))
    ...
    public function getTopSellingProducts(Request $request)
    {
        ...
        $data = DB::table('order_items')
            ...
            ->groupBy('products.id', 'products.name', 'product_images.image_path', 'products.price')
    ...
    {
        $recentOrders = Order::with('user:id,name')->orderBy('created_at', 'desc')->take(5)->get()->map(function($order) {
            $order->status = $order->order_status; // Map for frontend convenience
        ...
        return response()->json([
            ...
            'lowStockProducts' => $lowStockProducts, // Corrected variable name in response if needed, but sticking to existing structure
        ]);
    }
